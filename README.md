...
* **API 服务（当前为 hybrid 模式）**：内置基于 FastAPI 的服务端，支持安全鉴权、健康检查以及一键 Bundle 产物下载。
...
* **CLI**：可通过参数显式指定 `--label-mode hybrid`（规则为主，极速）或 `--label-mode react`（大模型深度介入）。
* **API**：当前接口仅接受 `label_mode=hybrid`（服务端参数已限制为 hybrid）。
...
# --- 扩展工具支持 (可选) ---
# 说明：当前版本 BING/GOOGLE 三项在代码中为固定空值，
# 暂未开放通过 key.env 动态读取；若需启用需先改配置读取逻辑。
BING_API_KEY=""
GOOGLE_API_KEY=""
GOOGLE_CX=""
