...
> 当用户明确提到“页边距/页眉页脚距离”时，可直接输出 page 字段，无需搜索。

- 页边距统一用厘米：
  - 例如“上3.0、下2.5、左2.6、右2.6厘米” →
    `{"page": {"margins_cm": {"top": 3.0, "bottom": 2.5, "left": 2.6, "right": 2.6}}}`
- 页眉页脚距离：
  - “页眉距边界2.0cm” → `{"page": {"header_distance_cm": 2.0}}`
  - “页脚距边界1.75cm” → `{"page": {"footer_distance_cm": 1.75}}`

---

## 九、模板中心与领域路由（新增）

> 模板选择是“先领域再细调”：先选基础 spec，再输出增量 overrides。

- `academic`：论文、学位、摘要、关键词、参考文献、APA/MLA、期刊投稿。
- `gov`：公文、红头、请示、通知、党政机关。
- `contract`：合同、协议、甲乙方、违约、签章、补充协议。
- 未明确时：保留当前模板或 default。

可选输出（用于模板路由）：
```json
{"_meta": {"domain": "gov", "spec_path": "specs/gov.yaml"}}
```
